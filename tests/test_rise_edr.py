...
def test_get_location(config):
    ...
    response = p.location(location_id='3196')
    assert len(response['domain']['axes']['t']['values']) == 100
    assert len(response['ranges']['50']['values']) == 100
...
def test_get_location_with_limit(config):
    ...
    response = p.location(location_id='3196', limit=10)
    assert len(response['domain']['axes']['t']['values']) == 10
    assert len(response['ranges']['50']['values']) == 10
    assert len(response['ranges']) == 1
...
def test_get_location_with_param(config):
    ...
    with pytest.raises(ProviderNoDataError):
        ...
        response = p.location(
            location_id='3196', select_properties=['not_a_real_parameter']
        )

    response = p.location(location_id='3196', select_properties=[50])
    assert len(response['domain']['axes']['t']['values']) == 100
    assert len(response['ranges']['50']['values']) == 100
    assert len(response['ranges']) == 1
...
def test_get_location_with_datetime(config):
    ...
    with pytest.raises(ProviderNoDataError):
        ...
        response = p.location(location_id='3196', datetime_='2020-01-01')

    response = p.location(
        location_id='3196', datetime_='2000-01-01T00:00:00Z/..'
    )
    assert len(response['domain']['axes']['t']['values']) == 90
    assert len(response['ranges']['50']['values']) == 90
    assert len(response['ranges']) == 1

    response = p.location(
        location_id='3196',
        datetime_='2000-01-01T00:00:00Z/2003-02-01T00:00:00Z',
    )
    assert len(response['domain']['axes']['t']['values']) == 74
    assert len(response['ranges']['50']['values']) == 74
    assert len(response['ranges']) == 1
